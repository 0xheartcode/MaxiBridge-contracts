...
│   ├── test/                 # 355/355 passing — BridgeEscrow + VestingVault + Mode4 + Create2Deploy + DecimalScaling + fork-tests-behind-SEPOLIA_RPC_URL
...
npm run test:all              # forge 355/355 + server 220/220 + opnet 52/52 + frontend 10/10
...
2. **Wrapped tokens:** unified — one `wUSDC`, one `wUSDT` on OPNet, 1:1 backed, 6 decimals (matches USDC/USDT). Future multi-chain expansion means same wUSDC is backed by all chains' USDC combined (fungibility risk to re-audit when adding BSC/Arb/Base). **Decimal convention for new flows:** stable pairs ship as 6/6 (matching the EVM-source decimals). EVM `WrappedERC20` accepts `decimals` as a constructor arg (not hardcoded 6) — the 6/6 default still applies for wUSDC/wUSDT deployments. Higher-precision assets (wDAI = 18, wWETH = 18, etc.) make a per-flow choice at `addFlow` time via `evmDecimals` / `opnetDecimals` in `FlowAddParams` — `AmountPolicy.quote` is decimal-aware so source and destination decimals can differ.

   **Flow modes (`TokenMode`):**
   - `0` WRAPPED — EVM lock → OPNet wrapped mint (USDC/USDT today).
   - `1` INVERSE_WRAPPED — OPNet lock → EVM wrapped mint.
   - `2` NATIVE_BURN_MINT — symmetric mint/burn, used for native tokens with bridge mint authority on both sides. **PVE003 (mode-2 launch hardening):** bridge-issued with no external backing, so its synthetic-supply ceiling is the wrapped token's **immutable `MAX_SUPPLY`** — enforced by `WrappedERC20.mintFromBridge` (EVM) and `OP20._mint` (OPNet) on EVERY mint path (`claim` AND `refundBurn`), and since burns reduce `totalSupply` it is a true OUTSTANDING-supply ceiling that frees headroom automatically (no bridge-side counter, no callback). To make that ceiling an unambiguous *per-flow* bound, `addFlow` enforces **1:1 token↔flow exclusivity** for mode 2 (a NATIVE_BURN_MINT token must back exactly one flow; modes 0/1/3/4 keep full N:M) via the `flowTokenMarker` (EVM) / `_flowTokenMarker` (OPNet) maps. **Deploy ceremony MUST set a mode-2 token's `MAX_SUPPLY` to the intended cap** — an uncapped token has no bound. This closes the PVE003 "refundBurn cap not binding" finding on BOTH chains. Chosen over a per-flow `cap` STATICCALL because the latter pushed `BridgeEscrow` past the EIP-170 size limit.
   ...
   **Custody vs mint is PER-MODE, not per-chain.** Whichever chain locks/pools the real asset is the *custody* side; whichever issues a wrapped token is the *mint* side — and it flips: mode 0 = EVM-custody / OPNet-mint; mode 1 = OPNet-custody / EVM-mint; mode 2 = mint-burn both; modes 3/4 = custody both. Mode 0 (USDC↔wUSDC) is the only EVM-custody-only / OPNet-mint-only flow, so don't over-generalize from it — this is exactly why OPNet mirrors EVM's `treasury`/`guardian`/`emergencyWithdraw` (it custodies real OP20 in modes 1/3/4). Full per-mode call-surface + cross-chain symmetry table: `docs/ARCHITECTURE.md` → "Token Mode, Custody/Mint Roles & Call Surface".

   **Routing is per-flow on BOTH chains (#68 — mode-per-flow).** The route's mode is read from the **flow** the transfer names, not from a per-token stamp. EVM was always per-flow (`lock`/`claim` carry `flowId`). OPNet now matches: `lockForBridge` reads `_flowMode[flowId]`; `claimMintWithVoucher` / `claimReleaseWithVoucher` read `_flowMode[voucher.flowId]` (the voucher carries `flowId` — Tier B, 540-byte preimage) and bind `_flowOpnetToken[flowId]==identity(wrappedToken)`; `burnForRelease` on both `WrappedOP20` and `WrappedERC20` takes a `flowId` first arg and emits it (Tier C). Net effect: **one (evmToken, opnetToken) pair can back several flows of different modes at once** (e.g. mode 3 AND mode 4), chosen per transfer by `flowId` — true N:M. The flow registry (governor-only `addFlow`) is the mode authority, and per-flow `status`/`cap`/`dailyLimit` now gate the voucher-claim leg too. Tier C changes the (non-upgradeable) `WrappedOP20.burnForRelease` selector → on mainnet a multi-mode mint/burn token needs a fresh wrapper deploy + holder migration + re-audit (do deliberately). Lock-initiated pooled multi-mode (Tier A) needs none of that.

3. **Upgradeability split:**
   - **EVM `BridgeEscrow`:** OZ UUPS proxy with full hardening (`_disableInitializers` in impl ctor, `initializer` gated init, `_authorizeUpgrade` onlyOwner, no `selfdestruct`, no arbitrary `delegatecall`, `uint256[40] __gap`; storage-layout CI diff gate is `astId`-insensitive). **Owner is `TimelockController(259200s)` (3 days), not the deployer EOA** — every upgrade goes through `schedule(...) → wait 3d → execute(...)`. Mirrors OPNet's `UpdatablePlugin(432 blocks)` so users have 3 days to exit on either chain. Deploy the timelock with `npm run deploy:evm:timelock` (script: `scripts/src/deploy/evm-deploy-timelock.ts`; default `259200`, override via `EVM_TIMELOCK_DELAY_SECONDS` but keep both chains in sync); transfer ownership with `npm run deploy:evm:transfer-ownership` (`scripts/src/deploy/evm-transfer-ownership-to-timelock.ts`). Proposer = governance Safe (`EVM_GOVERNANCE_SAFE`); executors = `address(0)` (open-execute, the 3-day delay IS the safeguard); admin = `address(0)` (burned at construction). Test scaffold: `contracts/evm-contracts/test/TimelockUpgrade.t.sol`. Ceremony walk-through: `docs/RUNBOOK.md` §0.
   - **OPNet `BridgeDepository`:** `UpdatablePlugin(432 blocks)` (~3 days at 10min/block) registered in the ctor — Phase 2.2 raised this from 144 (~24h); the 2026-05-27 governance decision set it to 432 (3 days) to match the EVM `TimelockController(259200s)` (was 1008/7d). `onUpdate()` runs migrations gated by `_storageVersion: StoredU256`; storage APPEND-ONLY per workspace "Five Upgrade Commandments". **Governance-gated upgrade authority (PR #43, closes Bug #16b):** in addition to the plugin's `onlyDeployer` gate, every applyUpdate requires governance pre-authorization. Governor (or registered BridgeAuthority) wires `setUpgradeAuthority(addr)`; from then on each upgrade needs `proposeUpgrade()` to land — `onUpdate` consumes the one-shot flag and reverts (rolling back the apply) if it isn't armed. Veto path: `cancelProposedUpgrade()` (governor / authority / upgradeAuthority). Legacy deployer-only path stays open while `_upgradeAuthority` is unset (v1 bootstrap window). Net effect: a compromised deployer hot key alone cannot push a malicious upgrade.
...
8. **`emergencyWithdraw` on `BridgeEscrow`** — Phase 1 hardened. Signature is now `(address token, uint256 amount)`; gated `guardian-only + whenPaused + nonReentrant` (sender MUST be `guardian`, not owner); drains exclusively to the `treasury` slot. Owner cannot drain unilaterally; the pause path must be exercised first. `treasury` and `guardian` are now **owner-rotatable** (`setTreasury` / `setGuardian`) — no longer set-once.
9. **M-of-N signer set on `BridgeEscrow`** (Phase 1.4). `mapping(address => bool) isSigner` + `signerCount` + `signerThreshold`. `claim()` (and `claimMintWrapped()`) require a **length-prefixed M-of-N signature blob** `[uint8 numSigs][sig_0(65)][sig_1(65)]…` — there is **NO legacy bare-65-byte path**. The deployed `BridgeEscrow` is *v2 — no legacy*: an unwrapped 65-byte sig reverts `InvalidSigBlob()`. A 1-of-1 deploy uses `numSigs=1` → a **66-byte blob**. The server wraps every ECDSA signature through `toClaimSigBlob()` (`server/src/voucher/evm-message.ts`) before it is stored as `ecdsa_signature` / sent to the contract. Distinct-signer enforced; `cancelledVouchers[opnetNonce]` blocks Tier-3 cancelled vouchers. Owner-rotatable `treasury` + `guardian` + dedicated freeze-only `pauser` complete the role lattice (see §14).
...
14. **Role-lattice parity — OPNet mirrors EVM (symmetric, same terminology both chains).** Both `BridgeEscrow` (EVM) and `BridgeDepository` (OPNet) expose the SAME named roles with the same semantics, so the two chains read identically. The shared lattice: owner/governor (transferable — OPNet `transferGovernor`, EVM `transferOwnership`), **rotatable** `treasury` + `guardian`, a dedicated freeze-only `pauser`, `emergencyWithdraw → treasury` (guardian + whenPaused), `withdrawFees → treasury` (onlyOwnerOrGuardian / onlyGovernorOrGuardian). **Freeze-but-never-thaw (audit H-01):** pause may be triggered by owner/governor, guardian, OR pauser; **unpause is owner/governor-only**.
    - **`treasury`** (set via `setTreasury` — **owner/governor-rotatable, NOT set-once**; `treasury()` view, `TreasurySet` event) — the PINNED sink for BOTH `withdrawFees` AND `emergencyWithdraw`. Fail-closed: a zero treasury blocks both. On OPNet, renamed from the interim `_feeRecipient`/`setFeeRecipient`/`FeeRecipientUpdated` (same storage pointer slot).
    - **`guardian`** (set via `setGuardian` — **rotatable**; `guardian()` view, `GuardianSet` event) — incident-response role. May FREEZE (`pause` / `setPaused(true)`), `cancelVoucher`, `removeSigner`, `migrateSignerSet`, and is the SOLE caller of `emergencyWithdraw`. May NOT unpause.
    - **`pauser`** (set via `setPauser` — rotatable, zero = disabled) — freeze-only; no other privileged surface.
    - **`emergencyWithdraw(token, amount)`** — `guardian-only` + whenPaused + `@nonReentrant`; transfers `amount` of `token` to `treasury` (reverts if treasury unset). The guardian+paused+treasury-pinned drain for custodied pooled/inverse (mode 1/3) balances, sitting ALONGSIDE the routine governor `drainInventoryOpNet` (PVE005-2: now treasury-pinned + `whenPaused`, the `recipient` param was DROPPED — mirrors EVM `BridgeEscrow.drainInventory`).
    - **`withdrawFees(flowId[, token], amount)`** — gate `onlyOwnerOrGuardian` (EVM) / `onlyGovernorOrGuardian` (OPNet); bounded by per-flow `accruedFees`; sweeps to `treasury`; does NOT require pause (routine revenue path).
    - **pause**: owner/governor / guardian / pauser may FREEZE; **unpause is owner/governor-only** (H-01).
    - **OPNet storage is a FRESH v1 baseline.** `onDeployment` ships `_storageVersion = 1` with EVERY role/refund field present (`StoredAddress`/`StoredMapU256` slots read zero = correct fail-closed base); the historical v2→v5 migration ladder is DELETED and `onUpdate`'s ladder is intentionally empty. The live deploy is a fresh stack, NOT an in-place upgrade of the prior mainnet depository (#68 Tier C forces fresh non-upgradeable wrappers, so the whole OPNet stack redeploys). Append-only discipline still governs any future post-launch upgrade, which starts a fresh ladder from this v1.
15. **Lock-refund — stranded-source-lock recovery, BOTH chains.** If a deposit/lock is locked on the source side but the destination voucher is permanently cancelled (reorg/fraud), the locked PRINCIPAL is returned to the original locker via a two-step M-of-N attested flow. **EVM:** `markDepositRefundable(depositNonce, sig)` (verifies a `RefundAuthorization(uint256 depositNonce,bytes32 flowId,uint32 signerEpoch)` EIP-712 attestation) → `refundLockedDeposit(depositNonce)` returns the locked tokens to the locker. **OPNet:** `markLockRefundable(lockNonce, sig)` → `refundLock(lockNonce)`; the OPNet attestation is **identity + reorg-guard bound** — the 264-byte `RefundAuthorization` preimage is REBUILT in-contract from the STORED lock record (`_lockToken`/locker/amount/lock time), so a refund can only ever target the exact original lock. **Inventory-reversal invariant:** a refund reverses the lock's inventory credit so the per-flow ledger stays consistent (`inventory + accruedFees == balance` holds across the refund). These return user principal (not mint authority) but still touch custodied balances — treat as a recovery path, exercised paused.
16. **Burn-side recovery (attested re-mint) — BOTH chains. MINT-AUTHORITY PRIMITIVE → DEDICATED RE-AUDIT BEFORE MAINNET.** Mirror of lock-refund for the BURN direction: when a burn's destination release is permanently cancelled, `refundBurn` RE-MINTS the burned amount to the original burner under M-of-N attestation. OPNet `refundBurn` (296-byte BurnRefundAuthorization) and EVM `refundBurn` (EIP-712 `BurnRefundAuthorization(address burner,address wrappedToken,uint256 amount,uint256 burnNonce,bytes32 burnTxHash,bytes32 burnBlockHash,bytes32 flowId,uint32 signerEpoch)`). Both: M-of-N attested, signer-attested `amount`, wrappedToken allowlist, flow binding, per-burn replay guard (`burnId`) set **before** the mint (CEI), and **both now `whenNotPaused` / `requireNotPaused`** (a freeze halts the re-mint path). See §8 for the full method-level write-up.
...
456     16    relayerTip          (u128 BE) — permissionless tip, PAID OUT on claim (see tip-payout note below)
...
**Migration note (PR β.2.format):** `grossSrcAmount` is plumbed equal to `grossDstAmount` until decimal-aware AmountPolicy lands.

**Relayer-tip payout — LIVE on BOTH chains (corrects the prior "signed but not paid" note).** The `relayerTip` field is signed over AND PAID OUT on claim. OPNet `claimMintWithVoucher` mint-splits (`recipientNet = netAmount - relayerTip`, mints the tip to `Blockchain.tx.sender` / relayer) and `claimReleaseWithVoucher` pool-splits; both emit `RelayerTipPaid(flowId, relayer, tip)`. EVM `claim` carves `recipientAmount = amount - tip`, `safeTransfer(msg.sender, tip)`, and emits `RelayerTipPaid`. Enforced against a per-flow `tipCapBps` (cap `MAX_TIP_BPS = 200` = 2%; default 0 = tipping disabled) using cross-multiplied (un-floored) ratio comparison; `tip <= amount` asserted. Front-run-safe: a tipped voucher opts into the relayer path; a tip-less (`relayerTip == 0`) voucher pays the named recipient in full.
...
| `setTreasury` (rotatable) | `setTreasury(address)` | `0xf0f44260` |
| `setGuardian` (rotatable) | `setGuardian(address)` | `0x8a0dac4a` |
| `setPauser` (rotatable; zero = disabled) | `setPauser(address)` | regenerated on rebuild |
...
| `withdrawFees` (→ treasury; onlyOwnerOrGuardian) | `withdrawFees(bytes32,uint256)` | regenerated on rebuild |
| `setFlowVestingVault` (Mode 4) | `setFlowVestingVault(bytes32,address)` | `0xc0f832a6` |
| `markDepositRefundable` (lock-refund; M-of-N attested) | `markDepositRefundable(uint256,bytes)` | regenerated on rebuild |
| `refundLockedDeposit` (lock-refund; returns locked principal to locker) | `refundLockedDeposit(uint256)` | regenerated on rebuild |
| `refundBurn` (#55 EVM — attested re-mint, modes 1/2; MINT-AUTHORITY, re-audit) | `refundBurn((address,address,uint256,uint256,bytes32,bytes32,bytes32,uint32),bytes)` | regenerated on rebuild |
...
| `drainInventoryOpNet` (#44 — flow-scoped; PVE005-2 treasury-pinned) | `drainInventoryOpNet(uint256,address,uint256)` — (flowId, token, amount) → `_treasury` | regenerated on rebuild |
...
| `refundBurn` (#55 — trustless burn-side recovery / attested re-mint; MINT-AUTHORITY, re-audit) | `refundBurn(bytes,bytes)` — (296-byte BurnRefundAuthorization preimage, M-of-N sig blob) | `0xbe782e17` |
| `isBurnRefunded` (view, #55; calldata widened by O-2 #109) | `isBurnRefunded(address,address,uint256,uint256)` — (wrappedToken, burner, burnTxHash, burnNonce) | regenerated on rebuild |
| `markLockRefundable` (lock-refund; M-of-N attested, 264-byte RefundAuthorization) | `markLockRefundable(uint256,bytes)` — (lockNonce, sig) | regenerated on rebuild |
| `refundLock` (lock-refund; returns locked principal to original locker) | `refundLock(uint256)` | regenerated on rebuild |
| `transferGovernor` | `transferGovernor(address)` | regenerated on rebuild |
| `setTreasury` (rotatable) | `setTreasury(address)` | regenerated on rebuild |
| `setGuardian` (rotatable) | `setGuardian(address)` | regenerated on rebuild |
| `setPauser` (rotatable) | `setPauser(address)` | regenerated on rebuild |
| `emergencyWithdraw` (→ treasury; guardian + paused) | `emergencyWithdraw(address,uint256)` | regenerated on rebuild |
| `withdrawFees` (→ treasury; onlyGovernorOrGuardian) | `withdrawFees(address,uint256)` | regenerated on rebuild |

> **#55 — trustless burn-side recovery (MINT-AUTHORITY PRIMITIVE — DEDICATED RE-AUDIT REQUIRED).** Closes the recovery gap for burn-initiated OPNet legs (mode 0 reverse: burn wUSDC → EVM release; mode 2 OPNet→EVM). If a burn's EVM destination voucher is PERMANENTLY cancelled (reorg/fraud), the burned tokens are gone AND the dest never pays → `refundBurn(attestation, sig)` RE-MINTS the burned amount to the original burner. Trust model == vouchers: the SAME M-of-N signer set attests (off-chain) that the burn is final AND the EVM dest voucher is cancelled-and-final. The depository has NO native burn record (the burn lives on the token), so the **attestation is self-contained** — the 296-byte BurnRefundAuthorization preimage (networkId‖contractSelf‖selector‖burner‖wrappedToken‖amount‖burnNonce‖burnTxHash‖burnBlock‖flowId‖signerEpoch) carries every binding field, and the M-of-N sig is verified over those exact bytes (no rebuild). **Per-burn replay guard** `burnId = sha256(wrappedToken‖burner‖burnTxHash‖burnNonce)` (O-2 #109 — `burnNonce` is a per-`WrappedOP20` counter, so the token + burner are bound in to keep the key collision-free across wrappeds; matches the EVM `BridgeEscrow.refundBurn` key + `docs/SECURITY.md` §6) is set strictly BEFORE the cross-contract `mintTo` (CEI) — it's the ONLY protection against a double/infinite re-mint. `amount` is signer-attested (same trust as a voucher's `netAmount`); `wrappedToken` must be allowlisted; `flowId` must exist + be ACTIVE + bind `_flowOpnetToken[flowId] == wrappedToken`. `@nonReentrant` + `requireNotPaused`. New append-only storage map `_refundedBurns` (no version bump — unset slot = not-refunded). 20 unit tests in `__test__/unit/tests/burn-refund.ts`. Ops: `npm run refund-burn:opnet` (`scripts/src/ops/refund-burn-opnet.ts`).

> **EVM-side symmetric burn recovery — NOW LIVE (MINT-AUTHORITY PRIMITIVE — DEDICATED RE-AUDIT REQUIRED).** `BridgeEscrow.refundBurn(BurnRefundAuthorization intent, bytes sig)` re-mints a burned `WrappedERC20` for the original burner on modes 1 (INVERSE_WRAPPED) / 2 (NATIVE_BURN_MINT) — the EVM mirror of OPNet `refundBurn`. EIP-712 struct `BurnRefundAuthorization(address burner,address wrappedToken,uint256 amount,uint256 burnNonce,bytes32 burnTxHash,bytes32 burnBlockHash,bytes32 flowId,uint32 signerEpoch)`. M-of-N attested over `_hashTypedDataV4`; `amount` signer-attested; `wrappedToken` must be `supportedToken`; flow must exist + be ACTIVE (M-3 — DRAINING rejected) + be a mint-on-EVM mode (1/2) + bind `flow.evmToken == wrappedToken`. Per-burn replay guard `burnId = keccak256(wrappedToken, burner, burnTxHash, burnNonce)` (M-4 — token + burner bound in; `burnNonce` is per-`WrappedERC20`, so two wrappeds can share a (txHash, nonce) pair) set strictly BEFORE `mintFromBridge` (CEI). Gated `nonReentrant + whenNotPaused` (a freeze halts the re-mint path — symmetric with OPNet `requireNotPaused`). Both chains' `refundBurn` are mint-authority primitives → **dedicated re-audit before mainnet.**
...
- UUPS hardening (all applied to `BridgeEscrow`):
  ...
  - `uint256[40] private __gap` trailing the storage layout — `50` baseline minus the `10` slots appended since (`unwrapFeeBps`, `unwrapMinFee`, `flows`, `allFlowIds`, `flowsByEvmToken`, `flowsByMode`, `flowsByEvmChain`, `lockedDeposits`, `pauser`, `refundedBurns`); matches the `uint256[40]` in `BridgeEscrow.sol` (slot 22 in `storage-layout.json` → `t_array(t_uint256)40_storage`). The committed `storage-layout.json` CI gate is the source of truth — reconcile this number against it before any upgrade.
